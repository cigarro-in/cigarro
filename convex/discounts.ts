...
// TEMPORARY one-shot backfill (public so PROD data can be seeded over HTTPS;
// removed before this wave ships — do not build on it).
export const backfillDiscounts = mutation({
  args: {
    rows: v.array(
      v.object({
        name: v.string(),
  ...
  handler: async (ctx, { rows }) => {
    ...
    let inserted = 0;
    for (const r of rows) {
      const code = r.code?.trim() ? r.code.trim().toUpperCase() : undefined;
      if (code) {
        const clash = await ctx.db
          .query("discounts")
          .withIndex("by_code", (q) => q.eq("code", code))
