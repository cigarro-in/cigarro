...
export function VariantsManager() {
  ...
  const fetchVariants = async () => {
    try {
      ...
      const { data, error } = await supabase
        ...
        .select('id, product_id, variant_name, variant_type, price, stock, attributes, is_active, created_at, updated_at, products(name, brand)')
  ...
  const handleDeleteVariant = async (variant: ProductVariant) => {
    if (!confirm(`Are you sure you want to delete variant "${variant.variant_name}"?`)) return;
  ...
  const columns = [
      ...
      render: (name: string, variant: ProductVariant) => (
        <div>
          ...
          <div className="text-sm text-gray-500">
            {variant.products?.name} - {variant.products?.brand}
          </div>
        </div>
      )
    },
    {
      ...
      label: 'Type',
      render: (t: string) => (
        <div className="text-xs uppercase tracking-wide">{t}</div>
      )
    },
    {
      key: 'price',
