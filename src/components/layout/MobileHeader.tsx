...
export const MobileHeader = ({ onMenuToggle, isMenuOpen }: MobileHeaderProps) => {
  ...
  return (
    <>
      ...
      <header className="md:hidden fixed top-0 left-0 right-0 z-50 p-2 safe-area-top">
        <div className="bg-creme border border-coyote rounded-md h-[50px] flex items-center justify-between px-2">
          {/* Menu Button with triple lines */}
          <button 
            type="button" 
            className="flex items-center justify-center h-[42px] w-[42px] rounded-md transition-colors duration-300 hover:bg-creme-light"
      ...
      {/* Search Sidebar (always mounted for smooth animation) */}
      <div
        className={`md:hidden fixed inset-0 z-50 transition-opacity duration-300 ${isSearchOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}
      ...
      >
        ...
        {/* Right-side drawer */}
        <div className="absolute inset-0 flex justify-end" onClick={(e) => e.stopPropagation()}>
          <div className={`h-full w-[90vw] max-w-sm bg-background border-l border-border shadow-xl transform transition-transform duration-300 ease-out will-change-transform flex flex-col ${isSearchOpen ? 'translate-x-0' : 'translate-x-full'}`}>
              {/* Drawer header with search input */}
                ...
                {showResults && (
                  <>
                    {searchResults.length > 0 ? (
                      <div className="p-4">
                        <div className="mb-4">
                          <h3 className="text-foreground font-sans font-semibold text-lg">
                        ...
                        <div className="space-y-3">
                          {searchResults.map((result) => (
                            <Link
                              ...
                              className="flex items-center gap-3 p-3 rounded-lg hover:bg-muted/50 transition-colors border border-transparent hover:border-border/20"
                              onClick={handleSearchClose}
                            >
                        ...
                        {searchResults.length >= 6 && (
                          <div className="mt-4 pt-4 border-t border-border">
                            <Link
                              ...
                              className="block text-center text-foreground font-sans font-medium text-base hover:text-accent transition-colors py-2"
                              onClick={handleSearchClose}
                            >
                      ...
                      <div className="p-8 text-center">
                        ...
                        <Link
                          to="/products"
                          className="inline-block bg-primary text-primary-foreground px-6 py-3 rounded-lg font-medium hover:bg-primary/90 transition-colors duration-300 text-sm uppercase tracking-wide"
                          onClick={handleSearchClose}
                        >
                          Browse All Products
                        </Link>
